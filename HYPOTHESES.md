...
**Test:** Monthly order count and revenue by category
(`mart_category_monthly_demand`), coefficient of variation across months
per category.

**Decision if true:** Time each category's buying to its own seasonal
cadence instead of one company-wide schedule, but weigh that against
holding-cost and demand risk (a category exposed to shifting consumer
sentiment, e.g. fur-adjacent fashion, carries real risk in buying ahead
of a seasonal peak).
...
trend (up or down) across the dataset's full time span, once overall
platform growth is netted out (see README "The confound I caught before
reporting H2").

**Test:** Linear trend fit (month index -> share of that month's total
items sold) per top category; categories with a statistically
significant, non-trivial slope are flagged growing/declining.
...
price and category roughly constant. In other words, "getting product to
market fast" is not just an internal ops metric, it's a demand-side lever
