...
df <- read.csv("data/processed/mart_order_satisfaction.csv", stringsAsFactors = TRUE)
df <- df[df$delivery_days >= 0 & df$delivery_days < 120, ]  # drop bad/extreme timestamps

model <- lm(review_score ~ delivery_days + price + freight_value + category, data = df)
s <- summary(model)
...
cat(sprintf("\nInterpretation: each additional day of delivery time is associated with a %.4f-point change in review score (1-5 scale), holding price/freight/category fixed. ",
            coefs["delivery_days", "Estimate"]))
if (coefs["delivery_days", "Pr(>|t|)"] < 0.001) {
  cat("This is highly statistically significant given the sample size, but note the practical size of the effect against R^2 below before treating it as a major lever.\n")
} else {
