...
| Products referencing a category missing from `category_translation` | **13 rows**, 2 real categories (`pc_gamer`, `portateis_cozinha_e_preparadores_de_alimentos`) genuinely missing from Olist's own translation table | Added the real missing translation explicitly (`src/03_migrate_to_supabase.py`); `pc_gamer` maps to itself (already an English/brand term), so that one is kept commented out in the script as a documented no-op rather than actively inserted |
